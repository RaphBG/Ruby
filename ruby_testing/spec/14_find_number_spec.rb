...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      let(:answer) { double('random_number', value: 5) }
      subject(:game) { described_class.new(3, 3, answer, 5) }
    ...
    context 'when guess and random_number are not equal' do
      let(:answer) { double('random_number', value: 5) }
      subject(:game) { described_class.new(3, 3, answer, 1) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'updates max to 8' do
        expect{high_guess_game.update_range}.to change{high_guess_game.max}.to(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      let(:answer) { double('random_number', value: 7) }
      subject(:guess_game) { described_class.new(5, 8, answer, 7) }
      it 'updates min to the same value as max' do
        expect{guess_game.update_range}.to change{guess_game.min}.to(8)
      end

      it 'does not update max' do
        expect{guess_game.update_range}.not_to change{guess_game.max}
      end
    end
  end
end
