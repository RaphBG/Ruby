...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        invalid = '12'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(invalid, valid_input)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid2 = '16'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(invalid1, invalid2, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = game.instance_variable_get(:@minimum)
        max = game.instance_variable_get(:@maximum)
        input = 5
        expect(game.verify_input(min, max, input)).to eq input
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game.instance_variable_get(:@maximum)
        input = 0
        expect(game.verify_input(min, max, input)).to eq nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game) { described_class.new(100, 600) }
      it 'returns 9' do
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game.display_turn_order(binary_search) }.to change { game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess).once
      game.display_turn_order(binary_search)
    end
