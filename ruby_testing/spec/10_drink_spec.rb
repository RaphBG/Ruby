...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_beverage) { described_class.new('tea') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_beverage.type).to eq('tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_beverage.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_juice) { described_class.new('orange_juice') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_juice).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_water) { described_class.new('water', 15) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_water).not_to be_full
      end
    end
  end
end
